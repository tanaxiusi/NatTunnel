...
typedef quint8 state_t[4][4];
static state_t* state;

// The array that stores the round keys.
static quint8 RoundKey[176];

// The Key input to the AES Program
static const quint8* Key;
...
/* Private functions:                                                        */
/*****************************************************************************/
static quint8 getSBoxValue(quint8 num)
{
  return sbox[num];
}

static quint8 getSBoxInvert(quint8 num)
{
  return rsbox[num];
}
...
static void AddRoundKey(quint8 round)
{
  quint8 i,j;
  for(i=0;i<4;++i)
  {
...
static void BlockCopy(quint8* output, quint8* input)
{
  quint8 i;
  for (i=0;i<KEYLEN;++i)
  {
...
static void XorWithIv(quint8* buf)
{
  quint8 i;
  for(i = 0; i < KEYLEN; ++i)
  {
...
{
  uintptr_t i;
  quint8 remainders = length % KEYLEN; /* Remaining bytes in the last non-full block */
